...
_key_resp_4_allKeys = []
# keep track of which components have finished
text_2Components = [text_3, key_resp_4]
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
text_2Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1
...
while continueRoutine:
    ...
    tThisFlip = win.getFutureFlipTime(clock=text_2Clock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
for thisInstrloop in Instrloop:
    ...
    _INSTRESP_allKeys = []
    # keep track of which components have finished
    FastInstrComponents = [Instr1, INSTRESP]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if INSTRESP.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > INSTRESP.tStartRefresh + 30-frameTolerance:
                # keep track of stop time/frame for later
                INSTRESP.tStop = t  # not accounting for scr refresh
                INSTRESP.frameNStop = frameN  # exact frame index
                win.timeOnFlip(INSTRESP, 'tStopRefresh')  # time at next scr refresh
                INSTRESP.status = FINISHED
    ...
    if INSTRESP.keys in ['', [], None]:  # No response was made
        INSTRESP.keys = None
    Instrloop.addData('INSTRESP.keys',INSTRESP.keys)
    if INSTRESP.keys != None:  # we had a response
        Instrloop.addData('INSTRESP.rt', INSTRESP.rt)
    Instrloop.addData('INSTRESP.started', INSTRESP.tStartRefresh)
    Instrloop.addData('INSTRESP.stopped', INSTRESP.tStopRefresh)
    thisExp.nextEntry()
...
for thisPractice in Practice:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if FTrue.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > FTrue.tStartRefresh + 6-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if JFALSE.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > JFALSE.tStartRefresh + 6-frameTolerance:
                # keep track of stop time/frame for later
...
for thisFastLoop in FastLoop:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if TRUE.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > TRUE.tStartRefresh + 6-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if FALSE.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > FALSE.tStartRefresh + 6-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if EncourageText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > EncourageText.tStartRefresh + 13-frameTolerance:
                # keep track of stop time/frame for later
        ...
        waitOnFlip = False
        if encouragekey.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisComponent in EncouragementComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    FastLoop.addData('EncourageText.started', EncourageText.tStartRefresh)
    FastLoop.addData('EncourageText.stopped', EncourageText.tStopRefresh)
    # check responses
    if encouragekey.keys in ['', [], None]:  # No response was made
        encouragekey.keys = None
    FastLoop.addData('encouragekey.keys',encouragekey.keys)
    if encouragekey.keys != None:  # we had a response
        FastLoop.addData('encouragekey.rt', encouragekey.rt)
    FastLoop.addData('encouragekey.started', encouragekey.tStartRefresh)
    FastLoop.addData('encouragekey.stopped', encouragekey.tStopRefresh)
    ...
    for thisComponent in AdvanceComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    FastLoop.addData('Continue.started', Continue.tStartRefresh)
    FastLoop.addData('Continue.stopped', Continue.tStopRefresh)
    # check responses
    if Advance_Resp.keys in ['', [], None]:  # No response was made
        Advance_Resp.keys = None
    FastLoop.addData('Advance_Resp.keys',Advance_Resp.keys)
    if Advance_Resp.keys != None:  # we had a response
        FastLoop.addData('Advance_Resp.rt', Advance_Resp.rt)
    FastLoop.addData('Advance_Resp.started', Advance_Resp.tStartRefresh)
    FastLoop.addData('Advance_Resp.stopped', Advance_Resp.tStopRefresh)
    thisExp.nextEntry()
...
# save data for this loop
FastLoop.saveAsExcel(filename + '.xlsx', sheetName='FastLoop',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
